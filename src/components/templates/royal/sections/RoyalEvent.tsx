...
const RoyalEvent = ({ data }: RoyalEventProps) => {
  ...
  const weddingDate =
    data.wedding_date || (acara.length > 0 ? acara[0].tanggal : null);
  ...
  const generateGoogleCalendarLink = () => {
    ...
    const namaPria = data.mempelai_pria?.nama?.split(" ")[0] || "";
    const namaWanita = data.mempelai_wanita?.nama?.split(" ")[0] || "";

    const title = encodeURIComponent(
      `The Wedding of ${namaPria} & ${namaWanita}`,
    );

    const location =
      acara.length > 0
        ? encodeURIComponent(`${acara[0].lokasi}, ${acara[0].alamat_lengkap}`)
        : "";
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="bg-white/80 backdrop-blur-md border border-pink-100 rounded-[2.5rem] p-8 shadow-xl relative overflow-hidden"
      >
        {/* Dekorasi Bunga Atas */}
        <motion.img
        ...
        <div className="text-center relative z-10">
          <motion.span
            ...
            className="text-pink-400 tracking-[0.4em] text-[10px] uppercase font-semibold"
          >
            Save the Date
          </motion.span>
          ...
          <motion.h2
            variants={itemVariants}
            className="text-4xl font-serif italic text-slate-800 mt-4 mb-2"
          >
            {weddingDate
          ...
          >
            ...
            <div className="text-5xl font-light text-pink-500">
              {weddingDate ? new Date(weddingDate).getDate() : "18"}
            </div>
            <div className="h-[1px] w-12 bg-pink-200" />
          </motion.div>
          ...
          <motion.p
            variants={itemVariants}
            className="text-slate-600 tracking-widest text-lg uppercase font-light"
          >
            {weddingDate
            ...
            {new Date(weddingDate || new Date()).getFullYear()}
          </motion.p>
        </div>
        ...
        <div className="mt-12 space-y-8 relative z-10">
          {acara.map((event, index) => (
            ...
            >
              <h4 className="font-serif text-xl text-slate-800 italic">
                {event.tipe}
              </h4>
              <p className="text-sm text-pink-400 mt-1 uppercase tracking-widest">
                {event.jam}
              </p>
        ...
        >
          <motion.button
            ...
            className="bg-pink-400 text-white px-8 py-3 rounded-full text-[10px] tracking-[0.2em] shadow-lg shadow-pink-100 uppercase"
          >
