      ...
      <Table>
        <TableHeader className="bg-[#FDFCFB]">
          <TableRow className="hover:bg-transparent border-[#F0EDE6]">
            <TableHead className="py-5 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-[#9B9B9B]">
              Detail Undangan
            </TableHead>
            <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-[#9B9B9B]">
              Jadwal Acara
            </TableHead>
            <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-[#9B9B9B]">
              Status
            ...
            <TableHead className="text-right px-6 text-[10px] font-black uppercase tracking-[0.2em] text-[#9B9B9B]">
              Opsi
            </TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {invitations.length > 0 ? (
            invitations.map((item) => {
              const eventDate =
                item.contentData?.acara?.[0]?.tanggal || "Belum diatur";

              return (
                <TableRow
                  key={item.id}
                  className="border-[#F0EDE6] hover:bg-[#FDFCFB]/50 transition-colors group"
                >
                  ...
                  <TableCell>
                    <div className="flex items-center gap-4">
                      <div
                        className="flex items-center gap-2 px-2.5 py-1 rounded-full border border-[#F0EDE6] bg-white"
                        title="Pesan Ucapan"
                      >
                        <MessageSquare className="size-3 text-[#D4AF97]" />
                        <span className="text-xs font-bold text-[#2C2C2C]">
                          {item._count.guestWishes}
                        </span>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="text-right px-6">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="secondary" size="icon" className="p-2 w-5 h-5 text-[#2C2C2C]">
                          ...
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent
                        align="end"
                        className="w-52 rounded-2xl border-[#F0EDE6] p-2 shadow-xl shadow-[#D4AF97]/5"
                      >
                        ...
                        <DropdownMenuItem
                          asChild
                          className="rounded-xl focus:bg-[#FDFCFB] focus:text-[#D4AF97] cursor-pointer"
                        >
                          <Link
                            href={`/edit/${item.id}`}
                            className="flex items-center gap-2 w-full py-2.5 font-bold text-xs text-[#2C2C2C]"
                          >
                        ...
                        >
                          <a
                            href={`/undangan/preview/${item.slug}`}
                            target="_blank"
