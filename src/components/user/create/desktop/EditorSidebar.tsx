    ...
    >
      ...
      <div className="p-6 space-y-6 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black tracking-tighter text-[#2C2C2C] uppercase leading-none">Editor</h2>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Undang Dong Engine</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={activeTemplate || "aura-dark"} onValueChange={setActiveTemplate}>
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                {musics.map((m) => (
                  <SelectItem key={m.id} value={m.url}>{m.title}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
        ...
        >
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ayah</Label>
                <Input 
                  value={formData.mempelai_pria?.ortu_ayah || ""} 
                  onChange={(e) => updateField("mempelai_pria", "ortu_ayah", e.target.value)}
                  className="rounded-xl bg-white"
                />
              </div>
              <div className="space-y-1.5">
                <Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ibu</Label>
                <Input 
                  value={formData.mempelai_pria?.ortu_ibu || ""} 
                  onChange={(e) => updateField("mempelai_pria", "ortu_ibu", e.target.value)}
                  className="rounded-xl bg-white"
                />
              </div>
            </div>
        ...
        >
          <div className="flex flex-col gap-3">
             <p className="text-[10px] text-slate-400 italic">Data acara akan muncul di sini sesuai format database.</p>
          </div>
        </SectionCard>
