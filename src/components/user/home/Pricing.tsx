...
const packages = [
  {
    ...
    features: [
      "10 Token / 1 Template pilihan",
      "Galeri foto 10 gambar",
      "Custom nama & tanggal",
      "Link undangan unik",
      "RSVP dasar / Daftar tamu hadir",
...
export default function Pricing() {
  return (
    <section
      id="harga"
      className="py-20 md:py-28 bg-white border-t border-[#E5E0D8]"
    >
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-12 md:mb-16">
          <span className="text-[#D4AF97] text-xs md:text-sm font-medium tracking-[0.2em] uppercase">
            PAKET HARGA
          </span>
          <h2 className="text-3xl md:text-5xl font-semibold text-[#2C2C2C] mt-4 tracking-tight leading-tight">
            Pilih paket yang sesuai
            <br className="hidden md:block" /> dengan kebutuhanmu
          </h2>
          <p className="text-[#6B6B6B] mt-4 max-w-md mx-auto text-sm md:text-base px-4">
            Semua paket sudah termasuk link unik, sistem RSVP, dan dukungan
            teknis.
          </p>
        </div>
  ...
  `}
            >
              ...
              <CardHeader className="text-center pt-12 pb-8">
                <CardTitle className="text-2xl md:text-3xl font-semibold text-[#2C2C2C]">
                  {pkg.name}
                </CardTitle>
                <div className="mt-4 flex flex-col items-center">
                  ...
                  <span className="text-xs md:text-sm text-[#6B6B6B] mt-1 italic">
                    / sekali bayar
                  </span>
                </div>
                <p className="text-[#6B6B6B] mt-3 text-sm px-4">
                  {pkg.description}
                </p>
              </CardHeader>
              ...
              <CardContent className="px-6 md:px-8 pb-10 flex-grow flex flex-col">
                <ul className="space-y-4 mb-10 flex-grow">
                  {pkg.features.map((feature, i) => (
                    <li
                      key={i}
                      className="flex items-start gap-3 text-[13px] md:text-sm text-[#2C2C2C] leading-tight"
                    >
        ...
        <div className="text-center mt-12 text-[10px] md:text-xs text-[#9B9B9B] leading-relaxed px-6">
          Semua harga sudah termasuk PPN • Pembayaran via QRIS • Konfirmasi
